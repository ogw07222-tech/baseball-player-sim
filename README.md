...
현재 공개 버전의 우선순위는 **무료 Player Career 경험 완성**이다.

시뮬레이션 코어는 장기적으로 동일한 야구 세계를 재사용해 여러 플레이 모드를 추가할 수 있도록 설계한다. 후보는 다음과 같다.
...
이 기능들은 **현재 공개 개발 범위가 아니며 지금 구현하지 않는다.** 현재 단계에서는 결제, 구독, entitlement, premium-only 시스템을 추가하지 않고 무료 Player Career와 재사용 가능한 simulation core를 우선한다.

향후 premium 기능은 추가 모드, 분석, 커스터마이징, 제어 범위를 확장하는 방식으로 설계하며, 선수 능력치·성장률·부상 확률·RNG·경기 확률 등 simulation outcome에 유료 우위를 주지 않는다.

새로운 일반 야구 시스템은 가능하면 Player Career 전용 객체에 종속시키지 않고 미래 GM / Manager / Sandbox / 자동 리그 시뮬레이션에서도 재사용 가능하도록 설계한다.

상세 장기 계획과 아키텍처 원칙은 [`docs/future-expansion-roadmap.md`](docs/future-expansion-roadmap.md) 참고.
