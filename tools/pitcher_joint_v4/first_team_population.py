...
def weighted_gameplay_mean(rows: list[WeightedSeason], attr: str) -> float:
    total = sum(r.pa for r in rows)
    return sum(getattr(r.gameplay(), attr) * r.pa for r in rows) / total
...
def main() -> None:
    ...
    raw={s:weighted_mean(rows,s) for s in names}
    gp={s:weighted_gameplay_mean(rows,s) for s in names}
    result={
        ...
        'pa_weighted_age_mean':sum(r.age*r.pa for r in rows)/total_pa,
