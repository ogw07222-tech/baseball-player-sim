...
def main():
 ...
 for i in range(a.careers):
  p=generate_pitcher(str(i),rng,player=False,age=18); rmax=p.stats.velocity;bmax=base_avg_kmh(rmax);emax=effective_avg_kmh(rmax,4.0,0.0);mmax=max_pitch_kmh(emax,max(0.0,rng.gauss(6.3,1.5)))
  while p.age<45:
 ...
 with open(a.output,'w',newline='') as f:
  ...
  for name,vals,cap in [('raw_velocity',raw,None),('base_avg_kmh',base,BASE_HARD_CAP),('effective_avg_kmh',eff,EFFECTIVE_HARD_CAP),('max_pitch_kmh',mx,MAX_PITCH_HARD_CAP)]:
   hit=0.0 if cap is None else sum(abs(v-cap)<1e-12 for v in vals)/len(vals)
   w.writerow([name,pct(vals,99),pct(vals,99.9),pct(vals,99.99),max(vals),hit])
