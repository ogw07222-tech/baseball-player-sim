...
def main():
 ...
 for i in range(a.n):
  x=generate_pitcher(str(i),rng,player=True,age=18)
  while x.age<28: apply_pitcher_season_growth(x,rng)
  for k in cols: cols[k].append(x.stats.current_ability() if k=='ability' else getattr(x.stats,k))
 out={'cohort':'player=True (Velocity v2 calibration cohort)','stats':{k:{'mean':statistics.fmean(v),'sd':statistics.pstdev(v)} for k,v in cols.items()}}
 import pathlib;pathlib.Path(a.output).parent.mkdir(parents=True,exist_ok=True);open(a.output,'w').write(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
