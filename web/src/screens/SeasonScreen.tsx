    ...
    {seasonTab==='리그 현황' && <>
      <div className="season-top-grid">
        <Panel title="팀 순위" className="standings-panel">
          <div className="subtabs"><button className="active">정규시즌</button><button disabled title="provider 계약 없음">최근 10경기</button><button disabled title="provider 계약 없음">홈 / 원정</button></div>
          {data.standings.length ? <div className="standings-table table-scroll"><table><thead><tr><th scope="col">순위</th><th scope="col">팀</th><th scope="col">승</th><th scope="col">패</th><th scope="col">무</th><th scope="col">승률</th><th scope="col">게임차</th><th scope="col">연속</th></tr></thead><tbody>{data.standings.map(row=><tr key={row.team} className={row.isUserTeam?'user-row':''}><td>{row.rank}</td><td>{row.team}</td><td>{row.w}</td><td>{row.l}</td><td>{row.d}</td><td>{fmt3(row.pct)}</td><td>{row.gb}</td><td>{row.streak}</td></tr>)}</tbody></table></div> : <div className="empty-tab">현재 리그 순위 데이터가 없습니다.</div>}
        </Panel>
        <Panel title="주요 타격 지표 리더보드" action={activeHitMetric ? <Tabs items={hittingMetrics} value={activeHitMetric} onChange={setHitMetric} ariaLabel="타격 지표"/> : undefined}><Leaderboard rows={activeHitMetric ? data.hittingLeaderboards[activeHitMetric] ?? [] : []}/></Panel>
        <Panel title="주요 투수 지표 리더보드" action={activePitchMetric ? <Tabs items={pitchingMetrics} value={activePitchMetric} onChange={setPitchMetric} ariaLabel="투수 지표"/> : undefined}><Leaderboard rows={activePitchMetric ? data.pitchingLeaderboards[activePitchMetric] ?? [] : []}/></Panel>
      ...
      <div className="season-bottom-grid">
        <Panel title={`${teamName} 팀 기록`} action={<Tabs items={['타격 기록','투수 기록','수비 기록']} value={teamTab} onChange={setTeamTab} ariaLabel="팀 기록 구분"/>}>
          {teamTab==='타격 기록'?(data.teamBatting.length?teamTable:<div className="empty-tab">현재 팀 타격 기록이 없습니다.</div>):<div className="empty-tab">{teamTab} 데이터는 현재 provider에 없으므로 생성하지 않습니다.</div>}
        </Panel>
        <Panel title="팀 주요 지표">{data.teamMetrics.length ? <div className="metrics-list">{data.teamMetrics.map(metric=><div key={metric.label}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.rank}위</small></div>)}</div> : <div className="empty-tab">현재 팀 주요 지표 데이터가 없습니다.</div>}</Panel>
      </div>
    </>}
    {seasonTab==='팀 기록' && <div className="season-bottom-grid"><Panel title={`${teamName} 팀 타격 기록`}>{data.teamBatting.length?teamTable:<div className="empty-tab">현재 제공되는 팀 타격 기록이 없습니다.</div>}</Panel><Panel title="팀 주요 지표">{data.teamMetrics.length?<div className="metrics-list">{data.teamMetrics.map(metric=><div key={metric.label}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.rank}위</small></div>)}</div>:<div className="empty-tab">현재 팀 주요 지표 데이터가 없습니다.</div>}</Panel></div>}
    {seasonTab==='선수 기록' && <>{dashboard?<><Panel title={`${dashboard.player.name} 시즌 기록`}><div className="record-hero-grid">{[['G',dashboard.seasonStats.g],['PA',dashboard.seasonStats.pa],['AVG',fmt3(dashboard.seasonStats.avg)],['OPS',fmt3(dashboard.seasonStats.ops)],['HR',dashboard.seasonStats.hr],['RBI',dashboard.seasonStats.rbi],['SB',dashboard.seasonStats.sb],['WAR',fmtWar(dashboard.seasonStats.war)]].map(([label,value])=><div key={String(label)}><span>{label}</span><strong>{value}</strong></div>)}</div></Panel><Panel title="타이틀 레이스"><Leaderboard rows={dashboard.titleRace.OPS ?? dashboard.titleRace.AVG ?? []}/></Panel></>:<div className="empty-tab panel">선수 기록 provider 데이터가 없습니다.</div>}</>}
    {seasonTab==='경기 일정/결과' && <Panel title="최근 경기 결과"><div className="recent-results">{data.recentResults.length?data.recentResults.map(game=><div key={`${game.date}-${game.awayTeam}`}><time>{game.date}</time><span>{game.awayTeam}</span><strong>{game.awayScore} : {game.homeScore}</strong><span>{game.homeTeam}</span><b className={game.result==='W'?'win':'loss'}>{game.result==='W'?'승':'패'}</b><small>{game.stadium}</small></div>):<div className="empty-tab">현재 제공되는 경기 결과가 없습니다.</div>}</div></Panel>}
