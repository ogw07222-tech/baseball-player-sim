...
export function CareerScreen({ data }: { data:DashboardViewModel }) {
  ...
  return <div className="overview-screen">
    ...
    <div className="overview-grid overview-grid-2">
      <Panel title="현재 능력치"><div className="overview-kv">{data.abilities.map(ability=><div key={ability.key}><span>{ability.label}</span><strong>{ability.rating}</strong><small>{ability.delta===0?'변화 없음':`${ability.delta>0?'+':''}${ability.delta}`}</small></div>)}</div></Panel>
...
export function LeagueScreen({ data }: { data:SeasonViewModel }) {
  return <div className="overview-screen">
    ...
    <div className="overview-grid overview-grid-2">
      <Panel title="타격 리더보드"><div className="overview-stack">{Object.entries(data.hittingLeaderboards).map(([metric,rows])=><section key={metric}><h3>{metric}</h3><Leaderboard rows={rows}/></section>)}</div></Panel>
      <Panel title="투수 리더보드"><div className="overview-stack">{Object.entries(data.pitchingLeaderboards).map(([metric,rows])=><section key={metric}><h3>{metric}</h3><Leaderboard rows={rows}/></section>)}</div></Panel>
    </div>
...
export function RecordsScreen({ dashboard, season }: { dashboard:DashboardViewModel; season:SeasonViewModel }) {
  ...
  return <div className="overview-screen">
    <div className="page-title-row"><div><h1>기록</h1><p>현재 provider에서 제공되는 시즌 기록을 한곳에 모았습니다.</p></div></div>
