...
class BattingLine:
    ...
    def add(self, other: "BattingLine") -> None:
        for name in ("G", "PA", "AB", "H", "doubles", "triples", "HR", "BB", "SO", "HBP", "SB", "CS", "R", "RBI"):
    ...
    def as_dict(self) -> dict[str, int]:
        return {
            ...
            "CS": self.CS, "R": self.R, "RBI": self.RBI,
        }

    @classmethod
    def from_dict(cls, data: dict[str, int]) -> "BattingLine":
        return cls(
            G=int(data.get("G", 0)), PA=int(data.get("PA", 0)), AB=int(data.get("AB", 0)), H=int(data.get("H", 0)),
            doubles=int(data.get("2B", data.get("doubles", 0))), triples=int(data.get("3B", data.get("triples", 0))),
            HR=int(data.get("HR", 0)), BB=int(data.get("BB", 0)), SO=int(data.get("SO", 0)), HBP=int(data.get("HBP", 0)),
            SB=int(data.get("SB", 0)), CS=int(data.get("CS", 0)), R=int(data.get("R", 0)), RBI=int(data.get("RBI", 0)),
        )
...
class SeasonRecord:
    ...
    def as_dict(self) -> dict[str, object]:
        return {"year": self.year, "age": self.age, "team": self.team, "first_team": self.first_team.as_dict(), "farm": self.farm.as_dict(), "awards": list(self.awards)}
