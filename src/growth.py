...
GROWABLE_STATS = tuple(name for name in config.STAT_NAMES if name != "talent")
...
def _normal_growth_delta(player: Player, stat_name: str, rng: RNG) -> int:
    ...
    # Physical attributes are more exposed to aging decline after age 31.
    if player.age >= 32:
...
def apply_season_growth(player: Player, rng: RNG) -> GrowthResult:
    ...
    if explosion:
        # One to three skills receive an additional large, uneven jump.
