...
def print_player(p:Player)->None:
    print('\n=== 선수 ===');print(f'{p.name} | {p.age}세 | {p.position} | {p.bats_throws}');print(f"소속: {p.team or '고교'} | 상태: {p.roster_level}")
...
def print_career_summary(p:Player)->None:
    total=p.first_team_career();best=p.best_season();print('\n=== 커리어 종료 ===');print(f'{p.name} | {p.position}');print(f"1군 데뷔: {p.debut_year if p.debut_year is not None else '없음'} | 은퇴: {p.retirement_age}세");print_line('KBO 1군 통산',total);counts={}
    for e in p.awards:
    ...
    print(f'초기 재능: {p.initial_talent}');print('최종 특성:',', '.join(t.name for t in p.traits) or '없음');print(f'커리어 이벤트: {len([e for e in p.event_history if e.get("event_id")!="coach_change"])}회')
...
def interactive_event_decider(event:CareerEvent,player:Player)->EventChoice:
    print('\n=== 커리어 이벤트 ===');print(event.name);print(event.description)
    for i,c in enumerate(event.choices,1):print(f'{i}. {c.name}')
    while True:
        raw=input('선택: ').strip()
...
def run_pro_cli(engine:CareerEngine,save_path:Path)->None:
    while engine.phase=='PRO':
        ...
        if choice in {'1','2','3'}:engine.advance_pro_games({'1':1,'2':7,'3':25}[choice])
            ...
            record,growth=engine.finish_pro_season(interactive_event_decider);print_line(f'{record.year} 1군',record.first_team);print('수상:',', '.join(record.awards) or '없음')
            for r in engine.last_event_resolutions:
                changes=', '.join(f'{STAT_LABELS.get(k,k)} {v:+d}' for k,v in r.stat_changes.items()) or '직접 능력치 변화 없음';print(f'[이벤트 결과] {r.event_name} / {r.choice_name} → {r.outcome_name} | {changes} | 부상 {r.injury_change}')
            print('성장 폭발:' if growth.explosion else '시즌 성장:',growth.deltas)
