...
class CareerSeasonMixin:
    def _reconsider_roster(self,s:ProSeasonSession)->None:
        ...
        if s.current_level=='FARM' and evaluation>=competition:
            ...
            if self.rng.random()<callup_chance:
                s.current_level='FIRST';self.player.roster_level='FIRST';self.player.debut_year=self.player.debut_year or self.year
        elif s.current_level=='FIRST' and line.PA>=30:
            established=self.player.first_team_career().PA+s.record.first_team.PA>=config.ESTABLISHED_FIRST_TEAM_PA
            margin=config.ESTABLISHED_DEMOTION_MARGIN if established else config.REGULAR_DEMOTION_MARGIN
            if evaluation<competition-margin:
                s.current_level='FARM';self.player.roster_level='FARM'
    @staticmethod
    ...
    def _event_candidates(self,s:ProSeasonSession,ctx:EventContext)->list[CareerEvent]:
        ...
        return out
    def _queue_event(self,s:ProSeasonSession,event:CareerEvent,ctx:EventContext)->None:
        s.pending_event_id=event.id;s.pending_event_game=ctx.game_number;s.pending_event_phase=ctx.season_phase
    def pending_event(self)->CareerEvent|None:
        s=self.current_session
        return EVENT_BY_ID[s.pending_event_id] if s and s.pending_event_id else None
    ...
    def _maybe_event(self,s:ProSeasonSession,event_decider:EventDecider|None,stop_on_event:bool,preseason:bool=False)->bool:
        ...
        phase='preseason' if preseason else self._season_phase(s.games_completed);ctx=self._event_context(s,phase,0 if preseason else s.games_completed)
        chance=config.EVENT_PRESEASON_CHANCE if preseason else config.EVENT_BASE_CHANCE_PER_GAME
