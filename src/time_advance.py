...
class AdvancePipelineState:
    ...
    def from_dict(
        ...
        *,
        ...
        # Old saves do not have this foundation state. Missing keys therefore
        # produce zero/default aggregates rather than invalidating the save.
        fallback = default_date or date(1970, 1, 1)
        ...
        completed = [
            SeasonStatLine.from_dict(_mapping(item))
            for item in _sequence(data.get("completed_seasons"))
        ]
        recent = [
            GamePerformance.from_dict(_mapping(item))
            for item in _sequence(data.get("recent_games"))
        ]
        return cls(
...
class AdvanceSummary:
    ...
    def rating_delta(self) -> dict[str, int]:
        ...
        return {
            key: int(self.after_ratings.get(key, 0)) - int(self.before_ratings.get(key, 0))
...
class AdvanceResultViewModel:
    ...
    def from_summary(cls, summary: AdvanceSummary) -> "AdvanceResultViewModel":
        team_record = (
            {
                "wins": summary.team_wins,
                "losses": summary.team_losses,
                "ties": summary.team_ties,
            }
            if summary.team_result_supported
            else None
        )
        ...
        return cls(
            period_label=summary.period_type,
            date_range=f"{summary.start_date.isoformat()}~{summary.end_date.isoformat()}",
            games_played=summary.games_played,
            hitter_period_line=_mapping(overall.get("hitter")),
            pitcher_period_line=_mapping(overall.get("pitcher")),
            team_record_delta=team_record,
            season_total_line=season,
            notable_events=tuple(event.as_dict() for event in summary.major_events),
            rating_changes=summary.rating_delta,
        )
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "games_played": self.games_played,
            "hitter_period_line": dict(self.hitter_period_line),
            "pitcher_period_line": dict(self.pitcher_period_line),
            "team_record_delta": dict(self.team_record_delta) if self.team_record_delta is not None else None,
            "season_total_line": dict(self.season_total_line),
            "notable_events": [dict(event) for event in self.notable_events],
...
class AdvanceOrchestrator:
    def __init__(
        ...
        game_provider: GameAdvanceProvider,
        *,
        rating_provider: RawRatingProvider | None = None,
    ...
    def advance_one_game(self) -> AdvanceSummary:
        ...
        if game_date is None:
            raise NoScheduledGameError("no scheduled game after current date")
        return self._advance_dates("GAME", self.state.current_date, game_date, [game_date])

    def advance_one_week(self) -> AdvanceSummary:
        start = self.state.current_date
        end = start + timedelta(days=7)
        dates = self.schedule.game_dates(start, end)
        return self._advance_dates("WEEK", start, end, dates)

    def advance_one_month(self) -> AdvanceSummary:
        start = self.state.current_date
        end = _add_one_calendar_month(start)
        dates = self.schedule.game_dates(start, end)
        return self._advance_dates("MONTH", start, end, dates)

    def get_weekly_summary(self) -> PeriodStatLine:
        start = self.state.current_date - timedelta(days=7)
        games = [g for g in self.state.recent_games if start < g.game_date <= self.state.current_date]
        return aggregate_game_performances(games)

    def get_monthly_summary(self, year: int, month: int) -> PeriodStatLine:
        games = [
            g
            for g in self.state.recent_games
            if g.game_date.year == year and g.game_date.month == month
        ]
        return aggregate_game_performances(games)
    ...
    def _advance_dates(
        ...
        before = self.state.season.copy()
        before_ratings = _rating_snapshot(self.rating_provider)
        before_roster = self.roster_provider() if self.roster_provider else None
        games: list[GamePerformance] = []
        events: list[CareerEventSummary] = []

        for scheduled_date in dates:
            if not start < scheduled_date <= end:
                raise ValueError("schedule returned date outside requested range")
            game = self.game_provider.advance_game(scheduled_date)
            if game.game_date != scheduled_date:
                raise ValueError("game provider returned mismatched date")
            self.state.add_game(game)
            games.append(game)
            events.extend(
                CareerEventSummary(game.game_date, "game", message)
                for message in game.notable_events
            )

        self.state.current_date = end
        after_ratings = _rating_snapshot(self.rating_provider)
        after_roster = self.roster_provider() if self.roster_provider else None
        roster_changes: tuple[str, ...] = ()
        if before_roster is not None and after_roster is not None and before_roster != after_roster:
            roster_changes = (f"{before_roster}->{after_roster}",)

        wins = sum(g.team_result == "W" for g in games)
        losses = sum(g.team_result == "L" for g in games)
        ties = sum(g.team_result == "T" for g in games)
        supported = bool(games) and all(g.team_result is not None for g in games)
        ...
        return AdvanceSummary(
            ...
            team_losses=losses,
            team_ties=ties,
            team_result_supported=supported,
            player_period_stats=aggregate_game_performances(games),
            season_before=before,
            season_after=self.state.season.copy(),
            before_ratings=before_ratings,
            after_ratings=after_ratings,
            roster_changes=roster_changes,
            major_events=tuple(events),
        )
...
class ListScheduleProvider:
    ...
    def __init__(self, dates: Iterable[date]) -> None:
        object.__setattr__(self, "dates", tuple(sorted(set(dates))))
...
def _rating_snapshot(provider: RawRatingProvider | None) -> dict[str, int] | None:
    if provider is None:
        return None
    payload = {str(k): int(v) for k, v in provider().items()}
    _assert_raw_rating_payload(payload)
    return payload


def _assert_raw_rating_payload(payload: Mapping[str, int]) -> None:
    forbidden = ("normalized", "gameplay_", "effective_")
    bad = [key for key in payload if any(token in key.lower() for token in forbidden)]
    if bad:
        raise ValueError(f"UI stat pipeline accepts raw/display ratings only: {bad}")
